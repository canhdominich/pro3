(function (angular, undefined) {
	...
	module.directive('customSelect', ['SelectService','$parse','$rootScope', '$compile', '$timeout', '$q', 'customSelectDefaults', function (SelectService,$parse,$rootScope, $compile, $timeout, $q, baseOptions) {
		...
		return {
			...
			link: function (scope, elem, attrs, controller) {
				...
				var itemTemplate = elem.html().trim() || '{{' + (match[2] || match[1]) + '}}',
					...
					dropdownTemplate =
					'<a style="padding-left: 50px;width: 100%;" class="dropdown-toggle" data-toggle="dropdown" href ng-class="{ disabled: disabled }">' +
						'<span>{{displayText}}</span>' +
						'<b></b>' +
					'</a>' +
					'<div class="dropdown-menu">' +
						'<div stop-propagation="click" class="custom-select-search">' +
							'<input class="form-control ' + attrs.selectClass + '" type="text" autocomplete="off" ng-model="searchTerm" />' +
						'</div>' +
						'<ul role="menu">' +
				...
				function configChildScope() {
					...
					childScope.select = function (item) {
						...
						SelectService.setClassId(item.classid)
						$rootScope
						.$broadcast('ClassForStudent');
