angular
		...
		.controller(
				...
				function($timeout, $http, $scope, $window, AuthService,
					...
					var getYearSchool = function() {
						$http.get('api/student/' + $scope.studentid + '/mark')
								.success(function(res) {

									SelectService.setListMark(res);
									// console.log("users sidebar:", AuthService
									// .getListUser());
									$rootScope.$broadcast('ListMark');
					...
					var getSubject = function() {
						$http.get('api/subject').success(function(res) {
					...
					var getSchoolYear = function() {
						$http.get('api/schoolyear').success(function(res) {

							$scope.schoolyear = res;
					...
					var findIndex = function(arr) {
						var res = null;
						$scope.subject.forEach(function(sub) {
							if (sub.name == arr[0]) {
								if (sub.grade == parseInt(arr[1], 10)) {
									res = sub.subjectid;
								}
							}
						});
					...
					var splitSchoolyear = function() {
						$scope.q.semester = null;
						$scope.q.schoolyear = null;
						var arr = $scope.year;
						if (arr.length == 1) {
							if (arr[0].split(" ")[0] == "Semester") {
								if (arr[0] == 'Semester 1')
									$scope.q.semester = 1;
								else if (arr[0] == 'Semester 2')
									$scope.q.semester = 2;

							} else
								$scope.q.schoolyear = arr[0];

						} else if (arr.length == 2) {
							$scope.q.schoolyear = arr[0];
							if (arr[1] == 'Semester 1')
								$scope.q.semester = 1;
							else if (arr[1] == 'Semester 2')
								$scope.q.semester = 2;
					...
					$scope.search = function() {
						// console.log("subject:",$scope.q.subjectid );
						// console.log("scho year:",$scope.q.schoolyear );

						if ($scope.subj != null
								&& (typeof $scope.subj !== "undefined")) {
							var arr = $scope.subj.split("-");
							$scope.q.subjectid = findIndex(arr);

						}

						if ($scope.year != null
								&& (typeof $scope.year !== "undefined")) {
							splitSchoolyear();

						}
						$http.get(
								'api/student/' + $scope.studentid
										+ '/mark/search', {

									params : {
										query : JSON.stringify($scope.q)
									}
